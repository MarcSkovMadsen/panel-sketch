...
"""
    ...
    command_string = f"pytest {test_files} --doctest-modules --cov={PACKAGE_FOLDER}"
...
@task()
def pylint(command, files=f"setup.py tasks {PACKAGE_FOLDER} tests"):
...
@task
def mypy(command, files=f"setup.py tasks {PACKAGE_FOLDER} tests"):
