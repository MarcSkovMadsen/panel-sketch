...
def __sort__(iterable, key=None, reverse=False):  # Used by py_sort, can deal with kwargs
    if key:
        iterable.sort(
            lambda a, b: 1 if key(a) > key(b) else -1
        )  # JavaScript sort, case '==' is irrelevant for sorting
    else:
...
class complex:
    ...
    def __log__(self):
        return complex(
            Math.log(Math.sqrt(self.real * self.real + self.imag * self.imag)),
            Math.atan2(self.imag, self.real),
        )
...
class __Terminal__:
    ...
    def print(self, *args, sep=" ", end="\n"):
        self.buffer = "{}{}{}".format(self.buffer, sep.join([str(arg) for arg in args]), end)[
            -4096:
        ]
